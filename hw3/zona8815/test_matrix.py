...
class GradingTest(unittest.TestCase):
    ...
    def test_correctness(self):
        size_temp1 = random.randint(1, 1000)
        size_temp2 = random.randint(1, 1000)
        size_temp3 = random.randint(1, 1000)
        
        mat1 = _matrix.Matrix(size_temp1, size_temp2)
        mat2 = _matrix.Matrix(size_temp2, size_temp3)
        
        for i in range(size_temp1):
            for j in range(size_temp2):
                mat1[i, j] = random.randint(1, 1000)

        for i in range(size_temp2):
            for j in range(size_temp3):
                mat2[i, j] = random.randint(1, 1000)
        
        res1 = _matrix.multiply_naive(mat1, mat2)
        res2 = _matrix.multiply_tile(mat1, mat2, 64)
        res3 = _matrix.multiply_mkl(mat1, mat2)
        
        for i in range(size_temp1):
            for j in range(size_temp3):
                assert res1[i, j] == res2[i, j] == res3[i, j]
