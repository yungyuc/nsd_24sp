...
'''
    ...
    with open('performance.txt', 'w') as fobj:
        ...
        w.write(f'Start multiply_tile (tile=16, repeat={repeat}), take min = ')
        tile16sec = minsec = min(tile_16.repeat(repeat=repeat, number=1))
        w.write(f'{minsec} seconds\n')

        w.write(f'Start multiply_tile (tile=32, repeat={repeat}), take min = ')
        tile32sec = minsec = min(tile_32.repeat(repeat=repeat, number=1))
        w.write(f'{minsec} seconds\n')

        w.write(f'Start multiply_tile (tile=64, repeat={repeat}), take min = ')
        tile64sec = minsec = min(tile_64.repeat(repeat=repeat, number=1))
        w.write(f'{minsec} seconds\n')
